...
- Test with bad inputs: empty fields, invalid data, injection payloads
...
- Save test scripts to `tests/` for reproducibility
- If all acceptance criteria pass, mark PASS even with minor cosmetic findings

### Fallback: File-Based Communication
If MCP tools are unavailable, fall back to:
- Poll `~/shared-comms/dev-to-qa/` every 30 seconds for new work
- Write results to `~/shared-comms/qa-to-dev/`
